...
export const EditModal = ({ open, onClose, clickedUser }) => {
  ...
  const handleSaveChanges = () => {
    if (formData.password === formData.confirmpassword) {
      ...
      let logOut = false
      if (
        user.username === clickedUser.username &&
        (formData.role === 'basic_user' || editedUser.password)
      ) {
        logOut = true
      }
      handleEdit(editedUser, logOut)
    } else {
      setShowErrMsg(false)
  ...
  const handleEdit = async (props, logOut) => {
    ...
    api
      .put(`http://localhost:4000/users/${clickedUser.username}`, props)
      .then((res) => {
        if (logOut) {
          handleLogout()
        } else {
              ...
              <form>
                <div className="mt-4">
                  <div>
                    ...
                    <input
                      aria-labelledby="edit-username-label"
                      type="text"
                      name="username"
                    ...
                    />
                    <div className="mt-4">
                      <label id="edit-password-label" className="block text-left">
                        Password
                      </label>
                      <input
                        aria-labelledby="edit-password-label"
                        type="password"
                        name="password"
                        onChange={handleChange}
                  ...
                  <div className="mt-4">
                    <label id="edit-confirm-label" className="block text-left">
                      Confirm password
                    </label>
                    <input
                      ...
                      className="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600"
                    />
                    <label className="block mt-4 text-left">Role:</label>
                    <select
                      id="role"
                      name="role"
                      onChange={handleChange}
